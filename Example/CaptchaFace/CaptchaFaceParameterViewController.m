...
@property (assign, nonatomic) BOOL isInCustomMode;
@end
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if (textField.text && textField.text.length > 0)
        {
            ...
            {
                textField.text = self.customData[indexPath.row];
            }
        }
        else if (!textField.text || textField.text.length == 0)
        {
            if (self.customData && self.customData.count > indexPath.row && ![self.customData[indexPath.row] isKindOfClass:NSNull.class])
            {
            ...
            self.customData[indexPath.row] = self.pickerData[0];
            }
        }
    }
...
{
    self.isInCustomMode = !self.isInCustomMode;
    ...
    self.editingTextField.inputAccessoryView.hidden = self.isInCustomMode ? YES : NO;
    //when coming from another screen, no cell selected
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField
{
    ...
    if (self.segmentedControl.selectedSegmentIndex == 0)
    {
        if (indexPath.row == 0)
        {
            ...
            self.editingTextField.inputAccessoryView = nil;
            self.editingTextField.inputView = nil;
        }
